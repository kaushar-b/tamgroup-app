...
export default function Desserts() {
  ...
  useEffect(() => {
    const unsub = subscribeSection('desserts', list => { setDishes(list); setLoading(false); });
    return unsub;
  }, []);
    ...
    <View style={s.container}>
      ...
      <ScrollView contentContainerStyle={s.list}>
        ...
        <View style={s.promoCard}>
          <Text style={s.promoTitle}>Want more desserts? Download Our App</Text>
          <Text style={s.promoSub}>Cakes, pastries & more from Gourmet Fine Pastries!</Text>
        ...
        {loading ? (
          ...
          <View style={s.center}>
            <Ionicons name="ice-cream-outline" size={56} color={RED} />
            <Text style={s.emptyTxt}>{search ? 'No matches' : 'No desserts yet'}</Text>
          </View>
        ) : (
          filtered.map(dish => (
...
const s = StyleSheet.create({
  ...
  promoCard:       { width: '100%', backgroundColor: '#fff', borderRadius: 20, padding: 22, marginBottom: 20, alignItems: 'center', elevation: 3, borderWidth: 1.5, borderColor: '#F5C6D6' },
